  ...
  return (
    <div className="relative min-w-button">
      ...
      >
        <div
          id="confirm-button"
          className={`pt-15px pb-15px pr-4 pl-4 ${
            enabled ? "active:pt-4 active:pb-14px" : ""
          } ${spinning ? "opacity-0" : ""}`}
        >
          {label}
        </div>
...
export const BackButton = ({
  label,
  onClick,
}: Weak<PropsUIButtonBack>): JSX.Element => {
...
export const ForwardButton = ({
  label,
  onClick,
}: Weak<PropsUIButtonForward>): JSX.Element => {
  return (
    <IconLabelButton
      icon={ForwardSvg}
      label={label}
...
export const BackIconButton = ({
  onClick,
}: Weak<PropsUIButtonIconBack>): JSX.Element => {
  return <IconButton icon={BackSvg} onClick={onClick} />;
};

export const ForwardIconButton = ({
  onClick,
}: Weak<PropsUIButtonIconForward>): JSX.Element => {
...
export const IconButton = ({
  icon,
  onClick,
}: Weak<PropsUIButtonIcon>): JSX.Element => {
  return (
    <div
